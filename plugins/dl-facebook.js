...
try {
await conn.reply(m.chat, `${lenguajeGB['smsAvisoEG']()}${waittt}`, fkontak, m)
const d2ata = await facebook.v1(args[0]);
let r2es = '';
if (d2ata.urls && d2ata.urls.length > 0) {
r2es = `${d2ata.urls[0]?.hd || d2ata.urls[1]?.sd || ''}`
}
conn.sendFile(m.chat, r2es, 'error.mp4', `${vidcap}\n${wm}`, m);
} catch (err1) {
try {
...
conn.sendFile(m.chat, VIDEO, 'error.mp4', `${vidcap}\n${wm}`, m);
} catch (err2) {
try {
const ress = await fg.fbdl(args[0]);
const urll = await ress.data[0].url;
await conn.sendFile(m.chat, urll, 'error.mp4', `${vidcap}\n${wm}`, m);
} catch (err3) {
try {
const res = await fbDownloader(args[0]);
for (const result of res.download) {
const ur = result.url;
await conn.sendFile(m.chat, ur, 'error.mp4', `${vidcap}\n${wm}`, m);
}
} catch (err4) {
try {
...
await conn.sendFile(m.chat, url3, 'error.mp4', `${vidcap}\n${wm}`, m);
} catch (err5) {
try {
...
await m.reply(`${lenguajeGB['smsAvisoFG']()}𝘾𝙊𝙐𝙇𝘿𝙉'𝙏 𝘿𝙊𝙒𝙉𝙇𝙊𝘼𝘿 𝙏𝙃𝙀 𝙑𝙄𝘿𝙀𝙊`)
handler.limit = 0
}}}}}}}}

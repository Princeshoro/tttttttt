...
async function addExif(audioBuffer, name, author) {
  try {
    ...
    // Save the modified audio to a file (optional)
    const outputPath = path.join(__dirname, 'modified_audio.mp3'); // Example output path
    fs.writeFileSync(outputPath, modifiedAudio);
